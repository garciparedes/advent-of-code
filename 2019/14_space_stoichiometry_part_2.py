...
class Cooker(object):
    ...
    def optimize_ore(self, maximum: int):
        fuel_min, fuel_max = 1, maximum
        while (fuel_max - fuel_min) > 1:
            median = (fuel_min + fuel_max) // 2
            cooker = Cooker(self.recipes, 'ORE')
            cooker.compute_needed('FUEL', required=median)
            if cooker.base_amount <= maximum:
                fuel_min = median
            else:
                fuel_max = median
...
def main():
    ...
    result = cooker.optimize_ore(ore_max)
