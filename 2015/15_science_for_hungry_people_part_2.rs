...
impl<'a> Cookie<'a> {
    ...
    fn optimize(&mut self, calories: i32, spoons: usize) -> i32 {
        return self.optimize_(calories, spoons, 0)
    }
    fn optimize_(&mut self, calories: i32, spoons: usize, i: usize) -> i32 {
        if spoons == 0 {
            return self.score(calories);
        }
        if i == self.len() {
            return 0;
        }
        ...
        for s in 0..=spoons {
            ...
            let a = self.optimize_(calories, spoons - s, i + 1);
            if best < a {
                best = a;
            }
        }
    ...
    fn score(&self, desired_calories: i32) -> i32 {
        ...
        for i in 0..self.len() {
            ...
            calories += self.quantities[i] * self.index[i].calories;
        }

        if calories != desired_calories {
            return 0;
        }
...
fn main() -> io::Result<()> {
    ...
    let ans = cookie.optimize(500, 100);
