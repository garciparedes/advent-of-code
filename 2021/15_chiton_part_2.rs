...
fn extend_board(old: Vec<Vec<u32>>, mult: usize) -> Vec<Vec<u32>> {
    let mut new = vec![vec![0; old[0].len() * mult]; old.len() * mult];
    for ii in 0..mult {
        for jj in 0..mult {
            for i in 0..old.len() {
                for j in 0..old[0].len() {
                    let k = ii * old.len() + i;
                    let l = jj * old[0].len() + j;
                    new[k][l] = old[i][j] + (ii + jj) as u32;
                    if new[k][l] > 9 {
                        new[k][l] -= 9;
                    }
                }
